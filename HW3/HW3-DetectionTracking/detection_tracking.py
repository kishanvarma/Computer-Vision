...
def particle_filter(v, file_name):
    ...
    while(1):
        ...
        frame_w, frame_h, frame_dimensions = frame.shape
        particles = particles.clip(np.zeros(2), np.array((frame_w, frame_h)) - 1).astype(int)
...
def optical_flow(v, file_name):
    ...
    x = c + w / 2
    y = r + h / 2
    p0 = np.array([[[x, y]], [[x + w / 5, y + h / 5]], [[x - w / 5, y - h / 5]], [[x + h / 10, y + h / 10]],
                   [[x - h / 10, y - h / 10]], [[x - w / 5, y + h / 5]]], np.float32)

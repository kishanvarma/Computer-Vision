...
def recompute():
    ...
    fg_cumulative_hist1 = cumulative_histogram_for_superpixels(fg_segments, color_hists)
    bg_cumulative_hist2 = cumulative_histogram_for_superpixels(bg_segments, color_hists)
    ...
    mask = pixels_for_segment_selection(superpixels,
                                        np.nonzero(graph_cut))  # dummy assignment for mask, change it to your result
...
mode = True  # if True, draw circle of different. Press 'm' to toggle to color
color1Flag = False
color2Flag = False
ix, iy = -1, -1
counter = 0
...
def draw_circle(event, x, y, flags, param):
    global ix, iy, drawing, mode, counter, color1Flag, color2Flag
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if drawing == True:
            if mode == True:
                ...
                if counter % 5 == 0 and color1Flag == True and color2Flag == True:
                    recompute()
                counter = counter + 1
            else:
                color2Flag = True
        ...
        if mode == True:
            ...
            counter = 0
            if color1Flag == True and color2Flag == True:
                recompute()
        else:
            color2Flag = True
            cv2.circle(clone, (x, y), 10, (0, 0, 255), -1)
